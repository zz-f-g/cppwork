...
int append_digit(int top, const int num, const int zeroflag)
{
    ...
    {
        result[top] = chnstr[2 * num]; // 2: 1 Chinese character takes up 2 bytes
        result[top + 1] = chnstr[2 * num + 1]; // last half of Chinese character
        top += 2;
    }
    return top;
}
...
int append_other(int top, const int index)
{
    ...
    result[top] = tenstr[2 * index];
    result[top + 1] = tenstr[2 * index + 1];
    top += 2;
    return top;
}
