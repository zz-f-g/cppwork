...
- ``cin.eof()`` 判断文件读取的最后一个字符是否是文件结束符 EOF，返回逻辑值
- ``cin.peek()`` 返回输入流中的下一个字符，不提取（遇到文件结束符则返回 EOF）
- ``cin.putback(char)`` 将字符常量或者变量送入输入流的头部
- ``cin.ignore(uint, char)`` 忽略缓冲区中指定个数的字符，直到遇到中止字符为止
