...
class mto10_net_tools {
	...
	public:
		...
		/* ---------------------------------------
		   ...
		   --------------------------------------- */
		/* 向服务器发送你的学号+密码信息 */
		int  send_parameter(const char* my_no, const char my_op, const char* my_pwd, const int row = 8, const int col = 8, const int id = -1, const int delay = 1000, const char *datestr = NULL);
		...
		int get_coordinate(char& row, int& col) const;	//取自动解坐标（从0,0开始，按先行后列顺序依次查找，只要有解就发送坐标，不判断是否最优解）
};
